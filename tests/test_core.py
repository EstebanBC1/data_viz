...
def test_bar_plot_is_solid_accent_with_no_texture():
    d = pd.DataFrame({"k": ["A", "B", "C"], "v": [5, 3, 4]})
    ax = dv.bar_plot(d, x="k", y="v")
    accent = dv.theme_tokens()["accent"].lower()
    faces = [plt.matplotlib.colors.to_hex(p.get_facecolor()).lower()
             for p in ax.patches]
    assert faces == [accent, accent, accent]         # one solid color, no rainbow
    assert len(ax.lines) + len(ax.collections) == 0  # no clipped motif fills
...
def test_hist_plot_has_bars_and_no_overlaid_line():
    import numpy as np
    ...
    assert len(ax.lines) == 0           # no curve drawn on top of the bars
